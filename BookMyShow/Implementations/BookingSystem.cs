...
//        public static void DisplaySeats(Show show, int screenno)
//        {
//            Console.WriteLine("\n");
//            WriteCentered($"Theatre : {show.Theatre.Name} | Screen : {screenno} | Show Date : {show.ShowDate:dd/MM/yyyy} | Show Time : {show.ShowTime:hh/mm tt}");
...
//            try
//            {
//                var duplicateSeats = seatNumbers.GroupBy(x => x).Where(g => g.Count() > 1).Select(y => y.Key).ToList();
//                if (duplicateSeats.Count > 0)
//                {
//                    throw new DuplicateSeatException($"Duplicate seats found: {string.Join(", ", duplicateSeats)}. Please choose unique seats.");
//                }
